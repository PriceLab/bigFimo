...
test_createFastaFileForFimo <- function()
{
   ...
   createFastaFileForFimo(tbl.regions.2, "smallTest2.fa", "hg38")
   x <- readDNAStringSet("smallTest2.fa")
   checkEquals(length(x), 2)

} # test_createFastaFileForFimo
#------------------------------------------------------------------------------------------------------------------------
runFimo <- function(fastaFileName, resultsDirectory, threshold=1e-4, pwmFile)
{
   # printf("--- running FIMO")
   FIMO <- "/usr/local/bin/fimo"
     # file.path(Sys.getenv("HOME"), "meme", "bin", "fimo")  # true on both hagfish & khaleesi
   MOTIFS <- pwmFile
...
test_runFimo <- function()
{
   ...
   resultsFile <- runFimo(fastaFileName, resultsDirectory, threshold=1e-2, pwmFile="/usr/local/data/human-jaspar2018-hocomoco-swissregulon.meme")
...
test_expandFimoTable <- function(tbl)
{
   ...
   tbl.directFromFimo <- get(load("/usr/local/data/tbl.fimoBeforeColumnsFixes.RData"))
...
test_fimoBatch <- function()
{
   ...
   tbl.match <- fimoBatch(tbl.regions, 1e-5, genomeName="hg38",
                          pwmFile="/usr/local/data/human-jaspar2018-hocomoco-swissregulon.meme")
